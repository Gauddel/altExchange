...
class LiquidityBalance extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            currency2TokenAddress : this.props.currency2TokenAddress,
            ...
            totalSupply : 0,
            balanceOf : 0,
            totalBalanceOfEther : 0,
            totalBalanceOfERC20 : 0,
            nameOfExchange : ''
        };
    ...
    componentWillMount(nextProps, nextState) {
        this.state.web3.eth.getAccounts().then((accounts) => {
            ...
            var currency2Contract = new this.state.web3.eth.Contract(this.state.abi, this.state.currency2TokenAddress);
            ...
            this.factoryContract.methods.getExchange(this.state.token).call().then((exchangeAddress) => {
                const exchangeContract = new this.state.web3.eth.Contract(this.exchangeABI, exchangeAddress);
                exchangeContract.methods.getEthToTokenInputPrice(this.state.web3.utils.toWei('1', 'ether')).call().then((price) => {
                    this.setState({
                        exchangeRate : Math.round(this.state.web3.utils.fromWei(price, 'ether')* 100)/100,
                    });
                });

                // Get balance of liquidity.
                this.state.web3.eth.getAccounts().then((accounts) => {
                    var mainAccount = accounts[0];
                    ...
                    exchangeContract.methods.name().call().then((name) => {
                        this.setState({
                            nameOfExchange : name,
                        })
                    })
                ...
                var currency2Contract = new this.state.web3.eth.Contract(this.state.abi, this.state.currency2TokenAddress);
                currency2Contract.methods.balanceOf(exchangeAddress).call().then((balance) => {
                    this.setState({
                        totalBalanceOfERC20 : this.state.web3.utils.fromWei((balance).toString(), 'ether'),
                    })
                });
                this.state.web3.eth.getBalance(exchangeAddress).then((balance) => {
                    this.setState({
                        totalBalanceOfEther : this.state.web3.utils.fromWei(balance.toString(), 'ether'),
                    });
                });
                exchangeContract.methods.totalSupply().call().then((balance) => {
                    this.setState({
                        totalSupply : balance,
                    })
                })
    ...
    render() {
        var ratio = this.state.balanceOf/ this.state.totalSupply;
        var depositedCcy1 = (Math.round(this.state.totalBalanceOfEther * ratio *100)/100).toFixed(2);
        var depositedCcy2 = Number(Math.round(this.state.totalBalanceOfERC20 * ratio *100)/100).toFixed(2);
        var balanceOfinEther = Number(Math.round(this.state.web3.utils.fromWei(this.state.balanceOf.toString(), 'ether')*100000)/100000).toFixed(5);
        ...
        return (<div>
            ...
            <div id="amountOfLiquidityOwned">
                <div className="tile" id="liquidityOwned">
                    <div className="tile is-parent">
                        <article className="tile is-child notification is-warning">
                            <p className="subtitle">Balance of Pooled Token</p>
                            <p className="title">{balanceOfinEther}</p>
                        </article>
                    </div>
                </div>
            </div>            
